...
- 默认持续自主推进，不等待人工确认。
- 默认所有命令自动执行，不做人工审批门槛。
- 单个任务完成后必须自动衔接下一个任务，不能以“等待用户输入”作为默认收尾。
- 默认保持长时间连续运行，直到项目完成、遇到真实阻塞，或仓库内出现无法判定的高返工决策。
- 能从仓库内容推断的事项，直接做合理假设并记录。
- 只有当继续推进会造成高概率大返工，且仓库内无法推断时，才允许停下询问。
- 实现、编译、测试、修复错误属于同一条工作链，不能只实现不验证。
- 读取、编辑、安装依赖、构建、测试、迁移、提交等命令默认直接串联执行，直到当前功能块闭环。
- 不将“命令风险高”单独作为暂停理由；只有业务方向无法判断且仓库内无依据时才允许询问。
- 对话结束不是默认动作；默认动作是根据状态文件继续领取并执行下一个明确任务。
...
- 状态文件已更新
- 已形成 Git 提交
- 完成后立即根据 `PROJECT_STATUS.md`、`ROADMAP.md` 和阶段 TODO 自动进入下一个任务
