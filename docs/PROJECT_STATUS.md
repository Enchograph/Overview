...
- 已完成 Android 正式签名接入说明：新增 `docs/ANDROID_RELEASE_CHECKLIST.md`，沉淀 keystore 配置、环境变量回退和发布前检查清单
...
继续收敛剩余发布说明，优先把正式签名缺口与未验证平台项明确到可执行清单。
...
- Android release APK、后端本地开发启动、后端启动烟测、可选正式签名配置、客户端主流程烟测、单脚本端到端验证与正式签名接入说明现已打通，当前剩余首个交付阻塞点集中在正式签名材料缺失
- Android release APK、后端本地开发启动、后端启动烟测、可选正式签名配置、客户端主流程烟测、单脚本端到端验证、正式签名接入说明与 Windows 验证说明现已打通，当前剩余首个交付阻塞点集中在正式签名材料缺失
- Android release APK、后端本地开发启动、后端启动烟测、可选正式签名配置、客户端主流程烟测、单脚本端到端验证、正式签名接入说明、Windows 验证说明与交付索引页现已打通，当前剩余首个交付阻塞点集中在正式签名材料缺失
