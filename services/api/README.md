...
当前也已补充邮箱注册/登录 API、密码哈希、session token 生成、受保护 planning 接口，以及带 OpenAI provider、Azure Speech 转写入口和稳定错误码的 AI 服务接口基础。
...
- `src/ai/`：AI 服务抽象、OpenAI provider、Azure Speech 转写与请求校验
...
- `/ai/transcribe` 接收 Base64 编码的短音频和 locale，使用 Azure Speech 转写后返回文本结果
